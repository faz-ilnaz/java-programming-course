...
public class StudentDaoImpl implements StudentDao{
    ...
    public Student findByEmail(String email) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                student.setContacts(findContacts(student.getEmail()));
            }
    ...
    public Contacts findContacts(String email) {
    ...
    public boolean add(Student student) {
        ...
        try {
            String querystring = "INSERT INTO students (  " +
                    "name, email, password, salt, lastname, birthday, groupname, laboratory, activity) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);" +
                    "INSERT INTO contacts (user_email) VALUES (?)";
            con = getConnection();
            ptmt = con.prepareStatement(querystring);
            ...
            ptmt.setString(10, student.getEmail());
            ptmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
            ok = false;
    ...
    public boolean update(Student student) {
        ...
        try {
            String querystring = "UPDATE students SET " +
                    "name =?, lastname = ?, groupname = ?, laboratory = ?, information = ?, activity = ?, birthday = ?  WHERE id = ?; " +
            ...
            ptmt.setLong(8, student.getId());
            if (student.getContacts() == null) {
                student.setContacts(new Contacts());
            }
            ptmt.setString(9, student.getContacts().getVk());
            ptmt.setString(10, student.getContacts().getGmail());
            ptmt.setString(11, student.getContacts().getTwitter());
            ptmt.setString(12, student.getContacts().getInstagramm());
            ptmt.setString(13, student.getContacts().getLinkedin());
            ptmt.setString(14, student.getEmail());
